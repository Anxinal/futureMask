...
between two tokens as a continuous value (regression). Reports MAE and
directional accuracy on the validation split.

The probe input is  q_i ⊙ k_j  (element-wise product of the query at
position i and the key at position j).

Usage:
  python run_relative_position_probe_regression.py \
      --checkpoint checkpoints/encdec_8C/checkpoint_last.pt \
      --data data-bin/wikitext-103 \
      --split valid \
      --probe-updates 5000 \
      --probe-layer -1 \
      --output reg_rel_probe_metrics.json
"""
...
def main():
    ...
    print(f"reg_rel_probe: {len(train_batches)} train batches available", flush=True)
    ...
    while step < args.probe_updates:
        for sample in train_batches:
            ...
            pred = probe(qk).squeeze(-1)  # [N]
            target = distances.float()
            ...
            step += 1
            if step % 200 == 0:
                print(f"reg_rel_probe step {step} loss {loss.item():.4f}", flush=True)
    ...
    with torch.no_grad():
        for sample in val_iter:
            ...
            pred_dist = probe(qk).squeeze(-1)  # [N]
            true_dist = distances.float()

            abs_errs.append((pred_dist - true_dist).abs().cpu().numpy())
            ...
            if nonzero.any():
                ...
                dir_total += nonzero.sum().item()
            total += distances.numel()

    mae = float(np.concatenate(abs_errs).mean()) if abs_errs else float("nan")
    dir_acc = dir_correct / max(1, dir_total)
    print(
        f"reg_rel_probe: MAE={mae:.3f} dir_acc={dir_acc:.3f} n={total}",
        flush=True,
    )
    with open(args.output, "w") as f:
        json.dump(
            {
                "reg_rel_probe_mae": mae,
                "reg_rel_probe_dir_acc": dir_acc,
                "reg_rel_probe_n": total,
            },
            f,
        )
